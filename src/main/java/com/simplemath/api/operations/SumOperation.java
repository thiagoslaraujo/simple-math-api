...
public class SumOperation extends Operation {
	...
	protected OperationDTO executeUnityStep() {
		...
		if(resultUnity > 9) {
			...
			log.info("Result is greater than 9 [unity: {}, extraUnity: {}]", operationDTO.getUnity(), operationDTO.getExtraUnity());
		} else {
			operationDTO.setNextStep(OperationStep.DOZEN.name());
	...
	protected OperationDTO executeDozenStep() {
		...
		log.info("Dozen Step: {} + {} + [extraUnity: {}] = {}", Integer.parseInt(this.a.getDozen()), Integer.parseInt(this.b.getDozen()), operationDTO.getExtraUnity(), result);
		...
		if(result > 9) {
			...
			log.info("Result is greater than 9 [dozen: {}, extraDozen: {}]", operationDTO.getDozen(), operationDTO.getExtraDozen());
		} else {
			operationDTO.setNextStep(OperationStep.HUNDRED.name());
	...
	protected OperationDTO executeHundredStep() {
		...
		log.info("Hundred Step: {} + {} + [extraDozen: {}] = {}", Integer.parseInt(this.a.getHundred()), Integer.parseInt(this.b.getHundred()), operationDTO.getExtraDozen(),  result);
		
		if(result > 9) {
			String value = String.valueOf(result);
			operationDTO.setExtraHundred(String.valueOf(value.charAt(value.length() - 2)));
			operationDTO.setHundred(String.valueOf(value));
			log.info("Result is greater than 9 [hundred: {}, extraHundred: {}]", operationDTO.getHundred(), operationDTO.getExtraHundred());
		} else {
